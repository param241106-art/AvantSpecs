...
describe('App routing', () => {
  ...
  it('navigates to the House page via the navbar', async () => {
    ...
    render(<App />);
    await user.click(screen.getAllByRole('button', { name: 'House' })[0]);
    expect(window.location.hash).toBe('#/house');
  });

  it('navigates to the Trade & Markets page and renders its content', async () => {
    const user = userEvent.setup();
    render(<App />);
    await user.click(screen.getAllByRole('button', { name: 'Trade' })[0]);
